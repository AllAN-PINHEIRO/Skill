...
class PerfilProfessor(models.Model):
    """
    Tabela exclusiva para dados do Professor (Bio e Contato).
    """
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name='perfil_professor')
    
    bio = models.TextField(blank=True, null=True, help_text="Sobre mim")
    telefone = models.CharField(max_length=20, blank=True, null=True)
    
    def __str__(self):
        return f"Perfil Prof: {self.user.username}"
...
class Certificado(models.Model):
    perfil = models.ForeignKey(PerfilAluno, on_delete=models.CASCADE, related_name='certificados')
